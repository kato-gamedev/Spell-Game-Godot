...
func _enter_tree() -> void:
	# --- ADDED: Automatically make Material Overrides unique ---
	for p in _get_particles():
		if is_instance_valid(p) and p.material_override != null:
			# Only duplicate if it's not already local to the scene
			if not p.material_override.resource_local_to_scene:
				p.material_override = p.material_override.duplicate()
				p.material_override.resource_local_to_scene = true
				
	for m in _get_meshinstances():
		if is_instance_valid(m) and m.material_override != null:
			if not m.material_override.resource_local_to_scene:
				m.material_override = m.material_override.duplicate()
				m.material_override.resource_local_to_scene = true
	# -----------------------------------------------------------
