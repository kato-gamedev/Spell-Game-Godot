...
[Tool]
public partial class AssetPaletteUi : PanelContainer
{
	...
	private Control _dynamicPreviewAssetButton;
	private DynamicPreviewPanel _dynamicPreviewPanel;
	...
	[Signal]
	public delegate void GenerateZooEventHandler(string generateZoo);
	
	[Signal]
	public delegate void ReloadAssetPreviewEventHandler(string assetPath);
	
	[Signal]
	public delegate void DynamicPreviewShownEventHandler(string assetPath);
	
	[Signal]
	public delegate void DynamicPreviewHiddenEventHandler(string assetPath, bool update);
	[Signal]
	...
	#endregion
	public void Init(DynamicPreviewPanel dynamicPreviewPanel)
	{
		_dynamicPreviewPanel = dynamicPreviewPanel;
		_dynamicPreviewPanel.InitUi();
		_dynamicPreviewPanel.Close += HideDynamicPreview;
	...
	{
		_dynamicPreviewPanel.ApplyTheme(baseControl);
		...
		var textureIcon = ResourceLoader.Load<Texture2D>("res://addons/assetplacer/ui/icons/DynamicPreview.svg");
		var centerViewIcon = baseControl.GetThemeIcon("CenterView", "EditorIcons");
		...
		_assetButtonRightClickPopup.AddEntry("Open Dynamic Preview", textureIcon, new Callable(this, MethodName.ShowDynamicPreview), Key.V);
		_assetButtonRightClickPopup.AddEnumEntry(PreviewPerspectiveContextMenuLabel, new Callable(this, MethodName.SetPreviewPerspective), PropertyUtils.EnumToStrings(new []{Asset3DData.PreviewPerspective.Custom}));
		...
		_tabRightClickPopup.AddEnumEntry(PreviewPerspectiveContextMenuLabel, new Callable(this, MethodName.SetLibraryPreviewPerspective), PropertyUtils.EnumToStrings(new [] {Asset3DData.PreviewPerspective.Custom}));
		...
		_tabRightClickPopup.AddEntry("Generate Asset Zoo", centerViewIcon, new Callable(this, MethodName.EmitPathSignal), SignalName.GenerateZoo);
	...
	private void UpdateAssetFilter(string text)
	{
		foreach (var button in _assetButtons.Values)
		{
			button.Visible = button.assetName.ToLowerInvariant().Contains(text.ToLowerInvariant());
		}
	}
	...
	private void OnLoadButtonPressed()
	{
		...
		AssetPlacerUi.ClampWindowToScreen(_loadAssetLibraryDialog, this);
	...
	public void ShowSaveDialog(string libraryName, bool changeName)
	{
		...
		AssetPlacerUi.ClampWindowToScreen(_saveAssetLibraryDialog, this);
	...
	private void AddAsset(Asset3DData data)
	{
		...
		button.ShowDynamicPreview += ShowDynamicPreview;
	...
	private void ShowDynamicPreview(string assetPath)
	{
		var button = _assetButtons[assetPath];
		_dynamicPreviewAssetButton = button;
		_dynamicPreviewPanel.Show();
		_dynamicPreviewPanel.ShowPreview(button);
		EmitSignal(SignalName.DynamicPreviewShown, assetPath);
	}
	
	private void HideDynamicPreview(string assetPath, bool updateAssetThumbnail)
	{
		_dynamicPreviewAssetButton = null;
		_dynamicPreviewPanel.Hide();
		EmitSignal(SignalName.DynamicPreviewHidden, assetPath, updateAssetThumbnail);
	}
	...
	public void SetAssetBroken(string assetPath, bool broken)
	{
		bool valid = _assetButtons.TryGetValue(assetPath, out var button);
		if (!valid) return;
		if (broken)
		{
			UpdateAssetPreview(assetPath, _brokenIcon, _brokenIcon, new Variant());
			button.TooltipText = $"Asset file not found. Was at: {assetPath}\nRight click for options.";
		}
	...
	public void InitDynamicPreview(ViewportTexture vpTexture, DynamicPreviewController dynamicPreviewController)
	{
		_dynamicPreviewPanel.InitViewport(vpTexture, dynamicPreviewController);
	}

	public void Cleanup()
	{
		_dynamicPreviewPanel.QueueFree();
	}
}
