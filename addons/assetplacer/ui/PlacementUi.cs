...
public partial class PlacementUi : PanelContainer
{
	...
	[Signal]
	public delegate void PlacementModeExpandedEventHandler();
	...
	private Vector3.Axis TextToAxis(string text) {
		return text.ToLowerInvariant().Substring(0,2) switch
		{
			"yz" => (Vector3.Axis.X),
			"xz"=> (Vector3.Axis.Y),
			"xy" => (Vector3.Axis.Z),
	...
	public Func<Transform3D, Vector3> AlignmentDirection
	{
		get
		{
			...
			return _alignmentDirectionOptionButton.GetItemText(_alignmentDirectionOptionButton.Selected).ToLowerInvariant().Substring(0,2) switch
			{
				"+x" => t=>t.Right(),
				"-x" => t=>t.Left(),
				"+y" => t=>t.Up(),
	...
	public void Init()
	{
		...
		// set selected for option buttons, as scene file has an incompatibility for this field between 4.1->4.2
		_planeOptionButton.Selected = 1;
		_placementModeOptionButton.Selected = 0;
		_alignmentDirectionOptionButton.Selected = 2;
		...
		_placementModeOptionButton.Pressed += () => EmitSignal(SignalName.PlacementModeExpanded);
		...
		_placementModeOptionButton.Clear();
		_placementModeOptionButton.AddItem(GetPlacementModeString(PlacementMode.Plane));
		_placementModeOptionButton.AddItem(GetPlacementModeString(PlacementMode.Surface));
		var placementModeString = _placementModeOptionButton.GetItemText(_placementModeOptionButton.Selected);
		if (_placementModesStrings.ContainsKey(placementModeString))
		{
			UpdatePlacementMode(_placementModesStrings[placementModeString]);
		}
		else
		{
			GD.PrintErr("Invalid placement mode: ", placementModeString);
		}
	}

	public void AddTerrain3DOption()
	{
		_placementModeOptionButton.AddItem(GetPlacementModeString(PlacementMode.Terrain3D));
	}

	public bool PlacementOptionButtonHasItem(PlacementMode mode)
	{
		return GetPlacementOptionButtonItemIdx(mode) != -1;
	}
	
	private int GetPlacementOptionButtonItemIdx(PlacementMode mode)
	{
		for (int i = 0; i < _placementModeOptionButton.ItemCount; i++)
		{
			if (GetPlacementModeString(mode) == _placementModeOptionButton.GetItemText(i)) return i;
		}
	...
	{
		var placementModeString = _placementModeOptionButton.GetItemText((int) index);
		if (_placementModesStrings.ContainsKey(placementModeString))
		{
			AssetPlacerPersistence.StoreSceneData(PlacementModeSaveKey, placementModeString);
			UpdatePlacementMode(_placementModesStrings[placementModeString]);
		}
		else
		{
	...
	private void UpdatePlacementMode(PlacementMode mode)
	{
		switch (mode)
		{
		 ...
		 EmitSignal(SignalName.PlacementModeChanged, (int) mode);
	}
	...
	public void OnSceneChanged()
	{
		...
		var placementModeString = AssetPlacerPersistence.LoadSceneData(PlacementModeSaveKey, GetPlacementModeString(PlacementMode.Plane), Variant.Type.String).AsString();
		PlacementMode placementMode;
		if (!_placementModesStrings.ContainsKey(placementModeString))
		{
			placementModeString = _placementModeOptionButton.GetItemText(0);
		} 
		placementMode = _placementModesStrings[placementModeString];
		if (!PlacementOptionButtonHasItem(placementMode))
		{
			if (_placementModesStrings[placementModeString] == PlacementMode.Terrain3D)
			{
				GD.PushWarning("To Enable Terrain3D placement, make sure that the Terrain3D plugin is enabled in ProjectSettings.");
			}
			placementMode = PlacementMode.Plane;
		}
		
		_placementModeOptionButton.Select(GetPlacementOptionButtonItemIdx(placementMode));
