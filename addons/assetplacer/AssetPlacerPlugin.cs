...
public partial class AssetPlacerPlugin : ContextlessPlugin
{
	...
	public const string PluginFolderPath = "res://addons/assetplacer";
	private const string PluginFilePath = $"{PluginFolderPath}/plugin.cfg";
	...
	public static string godotVersion = "4.4";
	...
	private void InitShortcuts(Shortcuts shortcutsObj)
	{
		...
		Settings.RegisterSetting(Settings.DefaultCategory, shortcutShiftRotation, 45f, Variant.Type.Float, PropertyHint.Range, "-180,180,degrees");
			...
			if (positionInfo.posValid && asset != null)
			{
					...
					positionInfo.pos != _lastPaintingAssetPos) // don't check same position twice
				{
							...
							!VectorUtils.AreApproximatelyEqualPositions(a.GlobalTransform.Origin, positionInfo.pos)))
					{
						...
						if (!IsPositionOccupied(asset, localPosition, _assetPalette.GetSelectedAssetMeshPathOrNull(), out _lastPaintingTooltip))
						{
	...
	{
		if (IsPositionOccupied(_assetPalette.Hologram, _assetPalette.GetSelectedAssetMeshPathOrNull(), out var tooltip))
		{
	...
	private bool IsPositionOccupied(Node3D node, string meshPathOrNull, out string tooltip)
	{
		return IsPositionOccupied(node, node.Position, meshPathOrNull, out tooltip);
	}
	
	private bool IsPositionOccupied(Node3D node, Vector3 localPosition, string meshPathOrNull, out string tooltip)
	{
		tooltip = "";
		...
		foreach (var child in children)
		{
			if (child is Node3D childNode3D)
			{
				...
				if (VectorUtils.AreTransformsEqualApprox(childNode3D.Transform, placingNodeTransform))
				{
					var isMesh = meshPathOrNull != null;
					// same asset
					if (isMesh ? childNode3D is MeshInstance3D mi && mi.Mesh.ResourcePath == meshPathOrNull : childNode3D.SceneFilePath == node.SceneFilePath)
					{
